...
[AddComponentMenu("Playground/Gameplay/Object Shooter Pool")]
public class ObjectShooterPool : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        pool = Pool.GetPool(prefabToSpawn);
    ...
    void Update()
    {
           ...
           && Time.time >= timeOfLastSpawn + creationRate)
        {
            ...
            var newObject = pool.Get(transform.position, Quaternion.identity) as Projectile;
            // Projectile newObject = Instantiate<Projectile>(prefabToSpawn);
            ...
            if (b == null)
            {
                b = newObject.gameObject.AddComponent<BulletAttribute>();
            }
